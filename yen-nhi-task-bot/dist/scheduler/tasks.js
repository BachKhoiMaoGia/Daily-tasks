...
export async function sendChecklist() {
    ...
    const msg = 'Checklist sáng:\n' + rows.map((r, i) => `${i + 1}. ${r.content}${r.due_date ? ' @' + r.due_date : ''}${r.due_time ? ' @' + r.due_time : ''}`).join('\n');
    await sendMessage(config.bossZaloId || '', msg);
}
export async function sendNearDue() {
    const now = new Date();
    const rows = db.prepare('SELECT * FROM tasks WHERE done = 0 AND due_date IS NOT NULL AND due_time IS NOT NULL AND (near_due_notified IS NULL OR near_due_notified = 0)').all();
    for (const r of rows) {
